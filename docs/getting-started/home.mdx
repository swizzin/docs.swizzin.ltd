...
Don't use `su` or something like `sudo -s` (which is worth noting is default under Ubuntu). You won't be fully logged in as root and certain environment variables not having the full perspective of root will cause failures.

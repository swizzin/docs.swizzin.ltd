...
4. Under `Path`, choose `Add a new path`. Enter the path for the folder you wish to add your Radarr movies to and click the green check mark to add the directory.
...
- Upgrade mono-based v0.2 to mono-based v3
    ...
    1. Change the Release branch by going to Settings -> General -> Updates (needs Advanced settings shown), set the Branch value to `develop` and then press the Save button.

...
```plaintext
https://localhost/nzbhydra/api
```

We need to edit this link to **remove https** and **add the port number**. Thus, if your port number was `5076`, your edited URL would look like this:
...
```plaintext main
Name: NZBHydra2
URL: use the api endpoint from above.
API: <copy and paste from NZBHydra2 UI>
```
4. Click `Test`. If all is good, click `Save`.
